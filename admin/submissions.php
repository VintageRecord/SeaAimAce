...
$stmt = $db->prepare('SELECT * FROM contact_submissions ORDER BY created_at DESC LIMIT ? OFFSET ?');
$stmt->execute([$per, $offset]);
$submissions = $stmt->fetchAll();

$detail = null;
if (isset($_GET['view']) && is_numeric($_GET['view'])) {
    $s = $db->prepare('SELECT * FROM contact_submissions WHERE id = ?');
...
<div class="card" style="margin-bottom:18px">
    ...
    <div class="card-body">
        <table style="width:auto">
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500">Name</th><td><?= h($detail['name']) ?></td></tr>
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500">Email</th><td><a href="mailto:<?= h($detail['email']) ?>"><?= h($detail['email']) ?></a></td></tr>
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500">Phone</th><td><?= h($detail['phone'] ?: '—') ?></td></tr>
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500">Company</th><td><?= h($detail['company'] ?: '—') ?></td></tr>
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500">Interest</th><td><?= h($detail['interest'] ?: '—') ?></td></tr>
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500">Date</th><td><?= h($detail['created_at']) ?></td></tr>
            <tr><th style="text-align:right;padding-right:20px;color:var(--text-muted);font-weight:500;vertical-align:top">Message</th><td style="white-space:pre-wrap;max-width:500px"><?= h($detail['message']) ?></td></tr>
        </table>
...
<div class="card">
    ...
    <div class="card-body" style="padding:0">
        ...
        <div class="table-wrapper">
            <table>
                <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Interest</th><th>Message</th><th>Date</th><th></th></tr></thead>
                <tbody>
